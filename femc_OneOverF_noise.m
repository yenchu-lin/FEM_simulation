function [img,noiseMask] = femc_OneOverF_noise(I,imginfo)
% Add 1/f noise to the bitmap

% Create random noise
imsize = round(size(I,2));
im_rand = rand(imsize);
freq_limit = imginfo.noisefreqlimit;

% Builds X and Y of distance from centre
stepsize = (freq_limit-1)/(imsize/2);
[X,Y] = meshgrid([ freq_limit-stepsize:-stepsize:1, 1:stepsize:freq_limit-stepsize ],...
                 [ freq_limit-stepsize:-stepsize:1, 1:stepsize:freq_limit-stepsize ]);
